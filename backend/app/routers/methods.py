...
@router.get("/{id}")
def get_method_by_id(graph_name: str, id: int, entrypoint: bool = False):
    return fetch_method(id, graph_name, entrypoint)


@router.get("/{id}/callers")
def get_all_method_callers(graph_name: str, id: int):
    return fetch_method_callers(graph_name, id)


@router.get("/{id}/callers/{caller_id}")
def get_method_caller(graph_name: str, id: int, caller_id: int | None = None):
    return fetch_method_callers(graph_name, id, caller_id)


@router.get("/{id}/callees")
def get_all_method_callees(graph_name: str, id: int):
    return fetch_method_callees(graph_name, id)


@router.get("/{id}/callees/{callee_id}")
def get_method_callee(graph_name: str, id: int, callee_id: int | None = None):
